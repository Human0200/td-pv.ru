...
<div class="catalog-compare swipeignore" <?=$bDelCookie ? 'data-delcookie' : '';?>>
    ...
    <div class="catalog-compare__inner loading">
        ...
        <div class="table_compare catalog-block">
            <?foreach ($arResult['SECTIONS'] as $arSection):?>
                <?$isMultipleComparableItems = count($arSection['ITEMS']) > 1;?>
                <div class="compare-sections__item<?=$arSection['ID'] === $activeTabId ? ' active' : '';?>" data-section-id="<?=$arSection['ID'];?>">
                                            ...
                                            foreach ($arResult['ALL_OFFER_FIELDS'] as $propCode => $arProp) {
                                                ...
                                                <li class="btn btn-transparent <?=$arProp['IS_DELETED'] != 'N' ? 'visible' : '';?> btn-xs">
                                                    <span onclick="CatalogCompareObj.MakeAjaxAction('<?=CUtil::JSEscape($arProp['ACTION_LINK']);?>')">
                                                        <?=TSolution::showSpriteIconSvg(SITE_TEMPLATE_PATH.'/images/svg/catalog/item_icons.svg#plus', 'plus fill-theme', [
                                                            'WIDTH' => 8,
                                                            'HEIGHT' => 7,
                                                        ]);?>
                                                        <?=GetMessage('IBLOCK_FIELD_'.$propCode);?>
                                                    </span>
                                                </li>
                                                <?php
                                            }
                    ...
                    <?php
                    $arUnvisible = ['NAME', 'PREVIEW_PICTURE', 'DETAIL_PICTURE'];

                    // make conditions array
                    $arShowFileds = $arShowOfferFileds = $arShowProps = $arShowOfferProps = [];

                    if ($arResult['SHOW_FIELDS']) {
                        foreach ($arResult['SHOW_FIELDS'] as $code => $arProp) {
                            if (!in_array($code, $arUnvisible)) {
                                $showRow = true;
                                if (!isset($arResult['FIELDS_REQUIRED'][$code]) || ($isMultipleComparableItems && $arResult['DIFFERENT'])) {
                                    $arCompare = [];
                                    foreach ($arSection['ITEMS'] as &$arElement) {
                    ...
                    if ($arResult['SHOW_OFFER_FIELDS']) {
                        foreach ($arResult['SHOW_OFFER_FIELDS'] as $code => $arProp) {
                            $showRow = true;
                            if ($isMultipleComparableItems && $arResult['DIFFERENT']) {
                                $arCompare = [];
                                foreach ($arSection['ITEMS'] as &$arElement) {
                                    ...
                                    <?if ($arShowOfferProps):?>
                                        <?foreach ($arShowOfferProps as $code => $arProp):?>
                                            <div class="catalog-compare__prop-line font_xs">
                                                ...
                                                <?endif;?>
                                                <?$currency = $arElement['OFFER_DISPLAY_PROPERTIES']['PRICE_CURRENCY']['VALUE'];?>
                                                <?if ($arElement['OFFER_DISPLAY_PROPERTIES'][$code]['DISPLAY_VALUE']):?>
                                                    <?=is_array($arElement['OFFER_DISPLAY_PROPERTIES'][$code]['DISPLAY_VALUE']) ? implode(', ', $arElement['OFFER_DISPLAY_PROPERTIES'][$code]['DISPLAY_VALUE']) : str_replace('#CURRENCY#', $currency, $arElement['OFFER_DISPLAY_PROPERTIES'][$code]['DISPLAY_VALUE']);?>
                                                <?else:?>
                                                    <span class="muted">&mdash;</span>
                                                <?endif;?>
                                            </div>
...
<script type="text/javascript">
    BX.Aspro.Utils.readyDOM(() => {
        ...
        InitOwlSlider(() => {
            const allSection = $('.compare-sections__item');

            allSection.each(function(index){
                const section = $(this);

                const $sliderProducts = section.find('.catalog-compare__items');
                const $sliderProps = section.find('.catalog-compare__props-slider');
                const $propsLines = $sliderProps.find(".catalog-compare__prop-line");
                const $sliderProductsItems = $sliderProducts.find(".owl-item");
                const $sliderPropsItems = $sliderProps.find(".owl-item");

...
(new TSolution\Product\Offers\Compare($arResult, $arParams))->add();
...
if ($arParams['USE_COMPARE_GROUP'] === 'Y') {
    if ($arSectionsIDs) {
        $arResult['SECTIONS'] = TSolution\Cache::CIBLockSection_GetList(['SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => ['TAG' => TSolution\Cache::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'GROUP' => ['ID'], 'MULTI' => 'N']], ['ID' => $arSectionsIDs, 'ACTIVE' => 'Y'], false, ['ID', 'NAME', 'IBLOCK_ID']);
    }

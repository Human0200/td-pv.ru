...
if (
    ...
    echo "<script>location.href = '".$arParams['CHANGE_PASSWORD_URL'].(strlen($arResult['BACKURL']) ? (strpos($arParams['CHANGE_PASSWORD_URL'], '?') ? '&' : '?').'backurl='.$arResult['BACKURL'] : '')."';</script>";
    return;
}

// need pageobject.js for BX.reload()
?>
<script>BX.loadScript(['<?=Bitrix\Main\Page\Asset::getInstance()->getFullAssetPath('/bitrix/js/main/pageobject/pageobject.js');?>']);</script>
            ...
            <div class="form-body">
                ...
                <form id="avtorization-form" name="system_auth_form<?=$arResult['RND'];?>" method="post" target="_top" action="<?=$arParams['AUTH_URL'];?>?login=yes">
                    ...
                    <?if ($arResult['CAPTCHA_CODE']):?>
                        ...
                        ?>
                        <div class="captcha-row clearfix form-group fill-animate captcha-row--margined">
                            <label for="FORGOTPASSWD_CAPTCHA<?=$rand;?>" class="font_13 color_999"><span><?= $captcha->isService() && $captcha->isActive() ? GetMessage('FORM_GENERAL_RECAPTCHA') : GetMessage('CAPTCHA_PROMT');?>&nbsp;<span class="required-star">*</span></span></label>

...
<?if($arResult['LIST']):?>
	<?if(!function_exists('showAsproSmartSeoTags')):?>
		<?function showAsproSmartSeoTags($arParams = [
			...
			<div class="landings-list__info <?=($bModile ? 'landings-list__info--mobiled visible-xs' : 'hidden-xs')?> <?=($bSlider ? 'swipeignore' : '')?>">
				...
				$i = $lastConditionId = 0;
				?>
				<div class="d-inline landings-list__info-wrapper from_smartseo <?=($bWithHidden ? 'last' : '');?> flexbox flexbox--direction-row flexbox--align-center <?=($bSlider ? 'with-slider' : 'flexbox--wrap');?> mobile-scrolled mobile-scrolled--items-auto mobile-offset">
					<?foreach($arParams['ITEMS'] as $arItem):?>
						<?if ($arParams['PARAMS']['SHOW_BY_GROUPS'] === 'Y'):?>
							<?if (
								...
								<?if($bHidden):?>
									<div class="landings-list__item font_14">
										<button class="landings-list__item--js-more btn--no-btn-appearance dark_link <?=$classOpened?>" data-opened="<?=$opened?>" data-visible="<?=$arParams['COUNT']?>">
							...
							<?if(
								...
								<?if(strlen($conditionName)):?>
									<div class="landings-list__section-title font_14 option-font-bold darken <?=$bModile ? 'landings-list__section-title--mobiled visible-xs' : 'hidden-xs'?>"><?=$conditionName?></div>
								<?endif;?>
							<?endif;?>
							<?$lastConditionId = $arParams['INFO'][$arItem['TAG_ID']]['FILTER_CONDITION_ID'];?>
						<?endif;?>
						...
						?>
						<div class="landings-list__item font_14 <?=$class?>">
							<div>
								<?if(strlen($url)):?>
									<?if($arItem['ACTIVE_URL'] == 'Y'):?>
										<span class="<?=$itemClass;?> landings-list__item--active bg-more-theme <?=($bActiveFilter ? 'landings-list__item--reset colored_more_theme_bg2_hover' : '')?>"><span><?=htmlspecialcharsback($arItem['NAME'])?></span>
											<?if($arItem['ACTIVE']):?>
									...
									<?else:?>
										<a class="<?=$itemClass;?> shadow-hovered shadow-hovered-f600 shadow-no-border-hovered" href="<?=$url?>"><span><?=htmlspecialcharsback($arItem['NAME'])?></span></a>
									<?endif;?>
								<?else:?>
									<span class="<?=$itemClass;?>"><span><?=htmlspecialcharsback($arItem['NAME'])?></span></span>
								<?endif?>
							</div>
						</div>
					<?endforeach;?>
	...
	<div id="aspro-smartseo-tags__wrapper_<?=$arResult['UNIQUE']?>" class="landings-list landings-list--smartseo <?=$templateName?> with-<?=$arParams['VIEW_TYPE']?> aspro-smartseo-tags__wrapper">
		...
		<?if($arParams['TITLE_BLOCK']):?>
			<div class="landings-list__title darken font_16"><?=$arParams['TITLE_BLOCK']?></div>
		<?endif;?>
		<?if($bInFilterShow):?>

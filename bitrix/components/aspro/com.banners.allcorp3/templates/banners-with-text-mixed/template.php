		...
		<?endif;?>
			...
			<div class="<?=$gridClass?>">
				<?foreach ($arResult['ITEMS'] as $arItem) {
					...
					<?ob_start();?>
						<?if($bTextBG && $url):?>
							<a class="banners-with-text-mixed-list__item-link item-link-absolute" href="<?=$url?>"></a>
						<?endif;?>
					...
					<div class="banners-with-text-mixed-list__wrapper grid-list__item <?=$widthItem;?>">
						<div class="banners-with-text-mixed-list__item <?=$itemClass;?>" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
							<?if ($url && !$bTextBG):?>
								<a href="<?=$url;?>" class="banners-with-text-mixed-list__item-link<?=($bItemOffset ? ' rounded-4' : '');?>" title="<?=($arItem['PREVIEW_PICTURE']['TITLE']?$arItem['PREVIEW_PICTURE']['TITLE']:$arItem['NAME']);?>">
							<?endif;?>
							<span class="banners-with-text-mixed-list__item-image <?=($url ? ' shine' : '');?> <?=$imageClass?>" style="background-image:url(<?=$arItem['PREVIEW_PICTURE']['SRC'];?>)"></span>
							<?if ($url && !$bTextBG):?>
								</a>
							<?endif;?>
							...
							<div class="banners-with-text-mixed-list__item-text<?=$itemTextClass;?>">
								...
								<div class="banners-with-text-mixed-list__item-title <?=$fontClass;?>">
									<?if($url):?>
										<a href="<?=$url;?>" class="dark_link color-theme-target" title="<?=($arItem['PREVIEW_PICTURE']['TITLE']?$arItem['PREVIEW_PICTURE']['TITLE']:$arItem['NAME']);?>">
									<?endif;?>
									<?=$arItem['NAME'];?>
									<?if($url):?>
										</a>

				...
				<?foreach ($arResult['ITEMS'] as $arItem) {
					...
					<div class="banners-img-list__wrapper grid-list__item">
						<div class="banners-img-list__item" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
							<?if ($url):?>
								<a href="<?=$url;?>" class="banners-img-list__item-link hover_blink" title="<?=($arItem['PREVIEW_PICTURE']['TITLE']?$arItem['PREVIEW_PICTURE']['TITLE']:$arItem['NAME']);?>">
							<?endif;?>
							<span class="not-eyed-images-off--hidden"><?=($arItem['PREVIEW_PICTURE']['TITLE']?$arItem['PREVIEW_PICTURE']['TITLE']:$arItem['NAME']);?></span>
							<span class="banners-img-list__item-image <?=($url ? ' shine' : '');?> <?=$imageClass?>" style="background-image:url(<?=$arItem['PREVIEW_PICTURE']['SRC'];?>)"></span>
							<?if ($url):?>
								</a>
							<?endif;?>
						</div>

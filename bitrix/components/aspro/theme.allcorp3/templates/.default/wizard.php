...
if($GLOBALS['USER']->IsAdmin() && $GLOBALS['USER']->CanDoOperation('edit_php')){
	if(\Bitrix\Main\Loader::includeModule(TSolution::moduleID)){
		if($obModule = CModule::CreateModuleObject(TSolution::moduleID)){
			$moduleClass = $obModule::moduleClass;
			$moduleTitle = $obModule->MODULE_NAME;
			$solutionName = $obModule::themesSolutionName;
			$partnerName = $obModule::partnerName;
			$templateName = TSolution::templateName;
		}
		...
		if(strlen($moduleClass) && class_exists($moduleClass)){
			...
			$bExists = $bInstalled || in_array(TSolution::moduleID.':'.$partnerName.':'.$solutionName, $arWizardsIds);
											...
											if($bCreateSite){
												if(!isset($arSites[$siteId])){
													if(strlen($siteDir = isset($_REQUEST['siteDir']) ? strval($_REQUEST['siteDir']) : false)){
														$arResult['URL'] = '/bitrix/admin/wizard_install.php?lang='.$lang.'&wizardName='.($bInstalled ? '' : TSolution::moduleID.':').str_replace('.', ':', TSolution::moduleID).'&siteId='.$siteId.'&createSite=Y&siteDir='.$siteDir.'&templateID='.$templateName.'&thematic='.$thematic.'&preset='.$preset.'&'.bitrix_sessid_get();
													}
													else{
												...
												if(isset($arSites[$siteId])){
													$arResult['URL'] = '/bitrix/admin/wizard_install.php?lang='.$lang.'&wizardName='.($bInstalled ? '' : TSolution::moduleID.':').str_replace('.', ':', TSolution::moduleID).'&siteId='.$siteId.'&createSite=N&templateID='.$templateName.'&thematic='.$thematic.'&preset='.$preset.'&'.bitrix_sessid_get();
												}
												else{

...
$arTab = array();
$arParams['SET_TITLE'] = $arParams['SET_BROWSER_TITLE'] = 'N';
...
<div class="element-list <?=$blockClasses;?> <?=$templateName;?>-template" data-block_key="<?=$bufferKey;?>">
	<div class="maxwidth-theme">
		<?
		$path = SITE_DIR.'/include/mainpage/text-under-title/elements-list.php';
		$bShowContent = Solution::checkContentFile($path) && ($arParams['SHOW_PREVIEW_TEXT'] !== 'N' && $arParams['SHOW_PREVIEW_TEXT'] !== false);
		$bShowTitle = strlen($arParams['TITLE']) && $arParams['SHOW_TITLE'];
		$bShowTitleLink = $arParams['RIGHT_TITLE'] && $arParams['RIGHT_LINK'];
		?>
		<?if ($arParams['SUBTITLE']):?>
			<div class="index-block__subtitle"><?=$arParams['SUBTITLE']?></div>
		<?endif;?>
		<div class="index-block__title-wrapper index-block__title-wrapper--with-center-block <?=($bShowContent ? '' : 'index-block__title-wrapper--mb-52');?>">
			<?if($bShowTitle):?>
				<div class="index-block__part--left">
					<h2 class="index-block__title switcher-title h3<?=($bShowContent ? ' line' : '')?>"><?=$arParams['TITLE']?></h2>
				</div>
			<?endif;?>

		<div class="index-block__part--center <?=!$bShowTitle ? "no_title_block" : ""?>">
		...
		<div class="index-block__part--right">
			<?if($bShowTitleLink):?>
				<a class="index-block__link dark_link stroke-theme-hover right_link_block" href="<?=preg_replace('/(?<!:)[\/]{2,}/', '/', SITE_DIR.$arParams['RIGHT_LINK'])?>" >
					<?=$arParams['RIGHT_TITLE'];?>

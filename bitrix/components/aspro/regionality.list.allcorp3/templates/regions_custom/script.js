...
function rcShowConfirm() {
    ...
    // Показываем только один раз за сессию — даже если закрыли крестиком
    if (sessionStorage.getItem('rc_confirm_shown')) return;
    sessionStorage.setItem('rc_confirm_shown', '1');
...
function rcHideConfirm() {
    ...
    if (el) el.remove();
    // НЕ сбрасываем rc_confirm_shown — чтобы при переходе на другую страницу не показывался снова
}
...
function rcShowUnavailable() {
    // Не показываем если уже есть на странице
    if (document.getElementById('rc-unavailable-overlay')) return;
    ...
    if (allowedIds.indexOf(currentCity.id) !== -1 || currentCity.id === defaultId) return;


    var defaultName = '';
    RC.cities.forEach(function(c) { if (c.id === defaultId) defaultName = c.name; });

    var overlay = document.createElement('div');
    overlay.className = 'rc-overlay';
    overlay.id = 'rc-unavailable-overlay';
    overlay.onclick = function(e) {
...
function rcHideUnavailable() {
    var el = document.getElementById('rc-unavailable-overlay');
    if (el) el.remove();
}

/**
 * Возвращает список городов для модалки/поиска.
 * Если у товара задана привязка — привязанные города + Нижний Новгород.
 * Если привязки нет — все города.
 */
function rcGetVisibleCities() {
    if (!window.RC_PAGE_ALLOWED_IDS || !window.RC_PAGE_ALLOWED_IDS.length) {
        return RC.cities;
    }

    var ids = window.RC_PAGE_ALLOWED_IDS.map(function(id) { return parseInt(id, 10); });
    var defaultId = parseInt(RC.defaultCityId, 10);

    // НН всегда присутствует в списке
    if (ids.indexOf(defaultId) === -1) {
        ids.push(defaultId);
    }

    return RC.cities.filter(function(c) {
...
function rcOpenModal() {
    ...
    var visibleCities = rcGetVisibleCities();
    var hasFilter = !!(window.RC_PAGE_ALLOWED_IDS && window.RC_PAGE_ALLOWED_IDS.length);
...
function rcSearch(val) {
    ...
    var hits = rcGetVisibleCities().filter(function(c) {

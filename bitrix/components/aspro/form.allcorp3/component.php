...
$arParams['LICENCE_TYPE'] = $arThemes['LICENCE_TYPE'];
$arParams['AGREEMENT_OPTION_CODE'] = '';
...
foreach(GetModuleEvents(TSolution::moduleID, 'OnAsproAdditionalForm', true) as $arEvent) {
    $FormHtml = ExecuteModuleEventEx($arEvent, [$arParams]);
}
if (!empty($FormHtml)) {
    echo $FormHtml;
...
if ($arThemes['USE_BITRIX_FORM'] == 'Y' && Bitrix\Main\Loader::includeModule('form')) {
    ...
    $bFormID = (int) $code;
    $arFilter = ['ACTIVE' => 'Y', 'SITE' => [SITE_ID]];
    if ($bFormID) {
        $arFilter['ID'] = $code;
    } else {
    ...
    if ($item = $rsForm->Fetch()) {
        ...
        if ($arParams["LICENCE_TYPE"] === 'BITRIX') {
            $arParams["AGREEMENT_OPTION_CODE"] = 'AGREEMENT_'.strtoupper($code).'_FORM';
        }

        $isOCBuy = strpos(strtolower($code), 'aspro_'.TSolution::themesSolutionName.'_quick_buy') !== false;
        if ($isOCBuy && $arThemes['SHOW_OFFER'] === 'Y') {
            $arParams['SHOW_OFFER'] = 'Y';
        }
    ...
    if (!$bCRMForm) {
        $APPLICATION->IncludeComponent(
            ...
            [
                ...
                'LICENCE_TYPE' => $arParams['LICENCE_TYPE'],
                'LICENCE_CHECKED' => $arParams['LICENCE_CHECKED'],
                'AGREEMENT_OPTION_CODE' => $arParams['AGREEMENT_OPTION_CODE'],
                'SHOW_OFFER' => $arParams['SHOW_OFFER'] ?? 'N',
                'LICENCE_TEXT' => $arParams['LICENCE_TEXT'],
    ...
    if (CModule::IncludeModule('iblock')) {
        ...
        if (!$bFormID) {
            ...
            $arIBlock = CIBlock::GetList(false, ['ID' => $arParams['IBLOCK_ID']])->Fetch();
        ...
        if (!$bCRMForm) {
            ...
            if ($bCache && $obFormCache->InitCache($arParams['CACHE_TIME'], $CACHE_ID, $CACHE_PATH)) {
                ...
                $rsProp = CIBlock::GetProperties($arParams['IBLOCK_ID'], ['SORT' => 'ASC'], ['ACTIVE' => 'Y']);
            ...
            $arMess = CEventMessage::GetList($arResult['SITE']['ID'], $order = 'desc', ['TYPE_ID' => $event_name])->Fetch();
            if (!is_array($arMess)) {
            ...
            $arMessAdmin = CEventMessage::GetList($arResult['SITE']['ID'], $order = 'desc', ['TYPE_ID' => $event_name_admin])->Fetch();
            if (!is_array($arMessAdmin)) {
                $em = new CEventMessage();
                $arMessAdmin = [];
                $arMessAdmin['ID'] = $em->Add([
                    'ACTIVE' => 'Y',
                    'EVENT_NAME' => $event_name_admin,
                    'LID' => [$arResult['SITE']['LID']],
                    'EMAIL_FROM' => '#DEFAULT_EMAIL_FROM#',
                ...
                foreach($arResult['QUESTIONS'] as $FIELD_CODE => $arQuestion) {
                    ...
                    if (isset($_REQUEST[$FIELD_CODE])) {
                        ...
                        if ($arQuestion['MULTIPLE'] == 'Y') {
                            $valFile = ['n0' => $arQuestion['DEFAULT'], 'n1' => $arQuestion['DEFAULT']];
                        }
                    ...
                    switch($arQuestion['FIELD_TYPE']) {
                        case 'hidden':
                            $html = '<input type="hidden" id="'.$arQuestion['CODE'].'" name="'.$arQuestion['CODE'].'" class="form-control ignore '.$required.' '.$phone.'" '.$placeholder.' value="'.htmlspecialcharsbx($val).'" />'.$icon;
                            break;
                        case 'text':
                            $html = '<input type="'.($arQuestion['CODE'] == 'EMAIL' ? 'email' : ($arQuestion['CODE'] == 'PHONE' ? 'tel' : 'text')).'" id="'.$arQuestion['CODE'].'" name="'.$arQuestion['CODE'].'" class="form-control '.$required.' '.$phone.'" '.$placeholder.' value="'.htmlspecialcharsbx($val).'" />'.$icon;
                            break;
                        case 'integer':
                            $html = '<input type="number" id="'.$arQuestion['CODE'].'" name="'.$arQuestion['CODE'].'" class="form-control '.$required.'" '.$placeholder.' value="'.htmlspecialcharsbx($val).'" />'.$icon;
                            break;
                        case 'date':
                            $html = '<input type="text" id="'.$arQuestion['CODE'].'" name="'.$arQuestion['CODE'].'" class="form-control date '.$required.'" '.$placeholder.' value="'.htmlspecialcharsbx($val).'" />'.$icon;
                            break;
                        case 'datetime':
                            $html = '<input type="text" id="'.$arQuestion['CODE'].'" name="'.$arQuestion['CODE'].'" class="form-control datetime '.$required.'" '.$placeholder.' value="'.htmlspecialcharsbx($val).'" />'.$icon;
                            break;
                        case 'html':
                            $cur_val = (isset($val['TEXT']) ? $val['TEXT'] : $val);
                            $html = '<textarea id="'.$arQuestion['CODE'].'" rows="3" name="'.$arQuestion['CODE'].'" class="form-control '.$required.'" '.$placeholder.'>'.htmlspecialcharsbx($cur_val).'</textarea>'.$icon;
                            break;
                        case 'elements':
                            $rsValue = CIBlockElement::GetList(['SORT' => 'ASC', 'ID' => 'ASC'], ['IBLOCK_ID' => $arQuestion['LINK_IBLOCK_ID'], 'ACTIVE' => 'Y'], false, false, ['ID', 'NAME']);
                            ...
                            break;
                        case 'list':
                            $rsValue = CIBlockProperty::GetPropertyEnum($arQuestion['CODE'], ['SORT' => 'ASC', 'ID' => 'ASC'], ['IBLOCK_ID' => $arParams['IBLOCK_ID']]);
                            $multiple = $arQuestion['MULTIPLE'] == 'Y' ? ' multiple ' : '';
                            $html = '<select id="'.$arQuestion['CODE'].'" name="'.$arQuestion['CODE'].($arQuestion['MULTIPLE'] == 'Y' ? '[]' : '').'" class="form-control '.$required.'" '.$multiple.$placeholder.'>';
                            while($arValue = $rsValue->Fetch()) {
                                $selected = '';
                    ...
                    foreach(GetModuleEvents(TSolution::moduleID, 'OnAsproIBlockFormDrawFields', true) as $arEvent) {
                        ExecuteModuleEventEx($arEvent, [$FIELD_CODE, &$arResult['QUESTIONS'][$FIELD_CODE]]);
                    }
                }
            }
            ...
            if (strlen($arResult['ERROR']) <= 0) {
                    ...
                    if (is_array($_REQUEST)) {
                        foreach($_REQUEST as $code => $value) {
                            if ($arResult['QUESTIONS'][$code]['FIELD_TYPE'] == 'html') {
                                $_REQUEST[$code] = ['VALUE' => ['TEXT' => $value, 'TYPE' => $arResult['QUESTIONS'][$code]['FIELD_TYPE']]];
                            } elseif ($arResult['QUESTIONS'][$code]['FIELD_TYPE'] == 'date') {
                                if (strlen($value)) {
                                    $objDate = new Bitrix\Main\Type\Date(str_replace(['-', '/', ' ', ':'], ['.', '.', '.', '.'], $value));
                                    $_REQUEST[$code] = ['VALUE' => $objDate->toString()];
                                } else {
                                    $_REQUEST[$code] = ['VALUE' => $value];
                                }
                            } elseif ($arResult['QUESTIONS'][$code]['FIELD_TYPE'] == 'datetime') {
                                if (strlen($value)) {
                                    $arDateTime = explode(' ', $value);
                                    $objDateTime = new Bitrix\Main\Type\DateTime(str_replace(['-', '/', ' ', ':'], ['.', '.', '.', '.'], $arDateTime[0]).' '.str_replace(['-', '/', ' ', ':'], [':', ':', ':', ':'], $arDateTime[1]));
                                    $_REQUEST[$code] = ['VALUE' => $objDateTime->toString()];
                                } else {
                                    $_REQUEST[$code] = ['VALUE' => $value];
                                }
                            }
                                ...
                                if (isset($_FILES[$FIELD_CODE]) && !$bMultiple) {
                                    ...
                                    foreach($_FILES as $key => $arFile) {
                                        if (strpos($key, '_n') !== false) {
                                            if (is_numeric(str_replace([$FIELD_CODE, '_', 'n'], '', $key))) {
                                                $code = explode('_', $key);
                                    ...
                                    foreach ($arResult['QUESTIONS'] as $FIELD_CODE => $arQuestion) {
                                        if ($FIELD_CODE !== $antiSpamHiddenFieldCode) {
                                            if ($arQuestion['FIELD_TYPE'] == 'list' || $arQuestion['FIELD_TYPE'] == 'checkbox') {
                                                ...
                                                $dbRes = CIBlockElement::GetList([], ['ID' => $RESULT_ID, 'IBLOCK_ID' => $arParams['IBLOCK_ID']], false, false, ['ID', 'PROPERTY_'.$FIELD_CODE]);
                                                while ($arItem = $dbRes->Fetch()) {
                                                    if ($arItem['PROPERTY_'.strtoupper($FIELD_CODE).'_VALUE']) {
                                                        ...
                                                        $format = 0;
                                                        $formats = [GetMessage('FORM_CT_NAME_b'), GetMessage('FORM_CT_NAME_KB'), GetMessage('FORM_CT_NAME_MB'), GetMessage('FORM_CT_NAME_GB'), GetMessage('FORM_CT_NAME_TB')];
                                                ...
                                                $dbRes = CIBlockElement::GetList([], ['ID' => $arQuestion['VALUE'], 'IBLOCK_ID' => $arQuestion['LINK_IBLOCK_ID']], false, false, ['ID', 'NAME']);
                                                while ($arItem = $dbRes->Fetch()) {
                                ...
                                if ($arParams['SEF_MODE'] == 'Y') {
                                    ...
                                    LocalRedirect(
                                        $APPLICATION->GetCurPageParam(
                                            ...
                                            '&RESULT_ID='.$RESULT_ID.
                                            '&formresult='.urlencode($arResult['FORM_RESULT']),
                                            ['formresult', 'strFormNote', 'IBLOCK_ID', 'RESULT_ID']
                                        )
                                    );
                ...
                $arResult = array_merge(
                    ...
                    [
                        ...
                        'CLOSE_BUTTON' => '<button class="'.$arParams['CLOSE_BUTTON_CLASS'].'" data-url="'.$APPLICATION->GetCurPageParam('', ['formresult', 'strFormNote', 'IBLOCK_ID', 'RESULT_ID', 'bxajaxid', 'AJAX_CALL', 'SEF_APPLICATION_CUR_PAGE_URL']).'">'.$arParams['CLOSE_BUTTON_NAME'].'</button>',
                    ]
                );
                ...
                if ($arResult['CAPTCHA_TYPE'] == 'HIDE') {
                    ...
                    $arResult['QUESTIONS']['CAPTCHA'] = [
                        'HTML_CODE' => "<textarea name='nspm' style='display:none;'>".htmlspecialcharsbx($val).'</textarea>',
                        'CAPTION' => '',
                        'FIELD_TYPE' => 'hidden',
                        'STRUCTURE' => [
                            [
